...
> **编译环境要求**：需要安装 Xcode（或与系统版本匹配的 Swift 工具链）——自 macOS 27 SDK 起 SwiftUI 的 `@State` 由宏实现。仅安装 Command Line Tools 时，可临时以 26.5 SDK 构建：
> ```bash
...
**Q：系统设置里“允许在菜单栏中显示”已打开，但菜单栏看不到 Sirius 图标？**  
A：系统开关只控制系统级可见性；若你使用了第三方菜单栏管理工具（Ice / Thaw / Bartender / Hidden Bar 等），它会把 Sirius 归入自己的“隐藏区”。请在该工具的设置中把 **Sirius** 拖到可见区域（或临时退出该工具验证）。Sirius 自身的菜单栏图标注册是正常的。

**Q：琥珀微光会改动我的系统「夜览 (Night Shift)」设置吗？**  
A：会。当琥珀微光真正应用暖色时，Sirius 会关闭系统级全局 Night Shift，以免外接屏幕一同发黄；Sirius 无法读取你之前的夜览状态，因此退出后如需夜览请在系统设置中手动开启。**未启用琥珀微光时完全不会触碰夜览设置。**
